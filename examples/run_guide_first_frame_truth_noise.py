...
结果输出：
- outputs/debug/guide_first_frame_truth_noise_0065pix_result.json
- outputs/debug/guide_first_frame_truth_noise_0065pix_error_audit.json
"""
...
def main() -> None:
    ...
    guide_cfg_path = Path("configs/guide_truth_noise_0065pix.yaml")
    ...
    print(f"Residual RMS (arcsec): {solution.residual_rms_arcsec:.4f}")
    ...
    if result["error_audit"].get("enabled", False):
        ...
        current_to_oracle = audit_summary["counterfactual_solutions"]["delta_components"][
            "current_to_oracle"
        ]
        print(
            "Current->Oracle total/non-roll/roll (arcsec): "
            "{total:.4f} / {non_roll:.4f} / {roll:.4f}".format(
                total=current_to_oracle["total_arcsec"],
                non_roll=current_to_oracle["non_roll_arcsec"],
                roll=current_to_oracle["roll_arcsec"],
            )
        )
    print("Per detector:")
    for detector_id, stats in result["detector_stats"].items():
        ...
        print(
            "  {det}: truth={truth} selected={cand} matched={matched} ref={ref} {transform_desc}".format(
                det=detector_id,
    ...
    output_path = Path("outputs/debug/guide_first_frame_truth_noise_0065pix_result.json")
    audit_path = Path("outputs/debug/guide_first_frame_truth_noise_0065pix_error_audit.json")
